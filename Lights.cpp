...
bool PointLight::cast_shadow_ray(const Vec3 &hitPoint, const SceneObject &obj, double shadowBias, Vec3 &out) const {
  ...
  return obj.intersect_ray(shadowRay,out) && (out-hitPoint).magnitude() < distance;
}
...
bool DirectionalLight::cast_shadow_ray(const Vec3 &hitPoint, const SceneObject &obj, double shadowBias, Vec3 &out) const {
...
bool SpotLight::cast_shadow_ray(const Vec3 &hitPoint, const SceneObject &obj, double shadowBias, Vec3 &out) const {
  Vec3 lightDirection = (position - hitPoint).normalize();
  Ray shadowRay(hitPoint + lightDirection * shadowBias, lightDirection);
  double distance = (position - hitPoint).magnitude();
  return obj.intersect_ray(shadowRay,out)  && (out-hitPoint).magnitude() < distance;
...
bool AmbientLight::cast_shadow_ray(const Vec3 &hitPoint, const SceneObject &obj, double shadowBias, Vec3 &out) const {
  return false;
}