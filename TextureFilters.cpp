...
Color BilinearFilter::get_filtered_color(std::shared_ptr<Texture> texture, const Vec2 &uvCoords) const {
  ...
  double di = i - ti;
  double dj = j - tj;
  Color colorA = texture->colors[ti][tj].to_color() * (1-di) + texture->colors[std::min(texture->height-1,ti+1)][tj].to_color()*(di);
  Color colorB = texture->colors[ti][std::min(texture->width -1,tj+1)].to_color() * (1-di) +
    texture->colors[std::min(texture->height-1,ti+1)][std::min(texture->width -1,tj+1)].to_color()*(di);
